...
import { updatePairDayData, updateTokenDayData, updateFXSwapDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = FXSwapFactory.load(FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  fxswap.totalLiquidityETH = fxswap.totalLiquidityETH.minus(pair.trackedReserveETH as BigDecimal)
  ...
  fxswap.totalLiquidityETH = fxswap.totalLiquidityETH.plus(trackedLiquidityETH)
  fxswap.totalLiquidityUSD = fxswap.totalLiquidityETH.times(bundle.ethPrice)
...
export function handleMint(event: Mint): void {
  ...
  fxswap.txCount = fxswap.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let fxswapDayData = updateFXSwapDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  fxswapDayData.dailyVolumeUSD = fxswapDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  fxswapDayData.dailyVolumeETH = fxswapDayData.dailyVolumeETH.plus(trackedAmountETH)
  fxswapDayData.dailyVolumeUntracked = fxswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  fxswapDayData.save()
